...
and $\overline{\rules}$ is a set of rule for guiding how the configuration is transited, at least contain rules \rulelab{GC}, \rulelab{MoreTries}, \rulelab{NoTries}, \rulelab{PreCom}, and \rulelab{Com} in \Cref{fig:q-pi-semantics}. 
...
Rule \rulelab{GC} defines the message transformation behavior of conveying a message from a node to another that might be an intermediate router node, with the consumption of a qubit in each of the parties and a slot time change happening in the global clock.
...
The above transitions evolves the configuration in \Cref{sec:qamsyntax}, we first apply rule \rulelab{GC}. The message $\qsend{1}{c}{m}^0$ in cell \cn{Cat} is conveyed to the $r_1$ cell with the new probability $0.5$, meaning that only $50\%$ change this communication is guaranteed.
