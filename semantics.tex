...
we have the rule applications \rulelab{GenChan} and \rulelab{FinChan} for step (1) and (3) above, with some possible granting rules, such as rule \rulelab{Grant} above.
Rule \rulelab{GenChan} defines the message transformation behavior of conveying a message from a node to another that might be an intermediate router node, with the consumption of a qubit in each of the parties and a slot time change happening in the global clock.
...
In rule \rulelab{GenChan}, it also moves the necessary information, such as the sender and receiver channel names as well as the message, in the \cn{comm} cell with the update of the time stamp $t'$ in the \cn{pred} cell to be the current global time,
indicating that at the current point, the system is conveying a message from a node to another.
After an intermediate granting step, rule \rulelab{FinChan} is applied on a configuration to empty the \cn{comm} cell, initialize the \cn{pred} cell by putting an \cn{false} value, and increment the global clock.
...
The above transitions evolves the configuration in \Cref{sec:qamsyntax}, we first apply rule \rulelab{GenChan}. The message $\qsend{1}{c}{m}^0$ in cell \cn{Cat} is conveyed to the $r_1$ cell with the new probability $0.5$, meaning that only $50\%$ change this communication is guaranteed.
...
We then apply rule \rulelab{Grant} to put a \cn{true} value in the \cn{pred} cell and the \rulelab{FinChan} rule application cleans up the cells, so the transition of the message from node $r_1$ to \cn{Dan} can happen through the last \rulelab{GenChan} rule application.
...
rules \rulelab{PreCom} and \rulelab{Com} define the steps except the intermediate granting step.
Rule \rulelab{PreCom} is applicable if the sending operation is already in its destination cell, and being paralleled with a same-channel message receipt process $\qrev{c}{x}.P$. After applying the rule, the message is processed as $P[m/x]$, and we also move the message $\qsend{p}{c}{m}^t$ to the communication cell $\cn{comm}$ with the current time stamp $t'$, so further user-defined rules can be applied on the message.
The above example first provides an \rulelab{PreCom} rule application showing the exactly procedure above, i.e, the message is moved to the \cn{comm} cell with the current global time $2$.
...
Every time when an action is in the \cn{comm} cell, which we know rule \rulelab{PreCom} is applied prior to the state, we apply the function $f$ in the new content cell \cn{tp} to the two time stamps $t$ and $t'$, and check if the message starting time $t$ and its delivery time $t'$ is within a threshold.
...
where $C'$ is the new declared configuration and the rule set $\overline{\rules'}$ contains the five rules in \Cref{fig:q-pi-semantics}, rule \rulelab{Grant} above, as well as rule \rulelab{TP}.
